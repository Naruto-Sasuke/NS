...
-- Basic options
cmd:option('-style_image', 'output/examples/inputs/starry_night_crop.png',
           'Style target image')
cmd:option('-style_blend_weights', 'nil')
cmd:option('-content_image', 'output/examples/inputs/bike.jpg',
...
cmd:option('-cluster_ratio',{0.04},'Prior to cluster_num,representing the ratio of cluster number to all pixels in certain style layer')
cmd:option('-cluster_method','kmeans','kmenas|others')
cmd:option('-padding',9,'padding')
cmd:option('-data_generate',1,'Whether to regenerate new data')
cmd:option('-stride',1,'stride of calculate grams in features')
...
cmd:option('-style_layers', 'relu3_1', 'layers for style')
...
-- its cluster gram is gram_tb[1][1].
  ...
  if params.data_generate == 0 then
    ...
    print(clu_nums_str)
    command = string.format("python 'test2.py' --cluster_nums %s --style_layers %d --image_num %d --cluster_method %s", clu_nums_str, nS, nP, params.cluster_method)
    os.execute(command)
        ...
        -- Necessary to div padding**2 in calculating grams
		...
		torch.save('data/cluster_data.dat',{kernels_tb,  grams_tb, counts_tb, labels_tb})		
  ...
  for i = 1, #cnn do
    if next_content_idx <= #content_layers or next_style_idx <= #style_layers then
      ...
      if name == style_layers[next_style_idx] then
        ...
        local loss_module = nn.StyleLoss(params.style_weight, grams_tb[next_style_idx], kernels_tb[next_style_idx], params, norm):float()
        loss_module:cuda()
        net:add(loss_module)
...
function StyleLoss:__init(strength, grams_value, kernels, params, normalize)
  parent.__init(self)
  self.normalize = normalize or false
  self.strength = strength
...
function StyleLoss:updateOutput(input)
  ...
  self.target = torch.zeros(self.point_num, self.c, self.c):typeAs(input)
  self.G = torch.Tensor(self.target:size()):zero():cuda()
  local zero_gram = torch.zeros(self.c, self.c):typeAs(input)
  ...
  for bias_i = 1, self.h, self.stride do
    for bias_j = 1, self.w, self.stride do
        ...
        local order = find_nearest_cluster(self.kernels, point, 'Cosine')
        --check selected cluster not non-pointed.
        -- How to optimize?
        local clu_gram = torch.Tensor():cuda()
        for i = 1,(#order)[1] do
            -- If the cluster is pointed, then the gram_value[i] should not be filled with zeros
            if not torch.equal(self.grams_value[i], zero_gram) then
              if bias%3 == 0 then  -- every 3 pixels
                local f3 = io.open('log3.txt','a+')
                f3:write(bias..': '..order[i]..'\n')
                if bias == self.w*self.h then
                    f3:write('*************\n')
                end
                f3:close()
              end
                print(string.format('updateOutput cluster:%d',order[i]))
                clu_gram = self.grams_value[{{order[i]},{},{}}]:clone():cuda()
                break
            else
                warrning('Try to set bigger cluster number.')
            end
        end
...
-- I think both are wrong, as it needs a currently unclear function to transfer grad_Patch to gradInput.
        -- It shouldn't directly attach grad_patch to gradInput, as if overlapping exists.
        -- patch gradient replace.
        --ex_gradInput[{{},{m_in_ex - half_pad, m_in_ex + half_pad}, {n_in_ex - half_pad, n_in_ex + half_pad}}]:add(grad_patch[{{sample_index},{},{},{}}])
        -- central point gradient replace
        ex_gradInput[{{},{m_in_ex}, {n_in_ex}}]:add(grad_patch[{{sample_index},{},{half_pad+1},{half_pad+1}}])
  ...
  -- patch Average
  -- local averageTemplate = getAverageTemplate(ex_gradInput, self.padding)
  -- ex_gradInput = ex_gradInput:cdiv(averageTemplate)
