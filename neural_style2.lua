-- ***********************************************************
-- *********************** ORIGINAL  VERSION ******************
--TODO: cluster seem to return the same every time.
-- This is the original method backup.
...
cmd:option('-padding',7,'padding')
cmd:option('-data_generate',1,'Whether to regenerate new data')
cmd:option('-stride',7,'stride of calculate grams in features')
...
function StyleLoss:updateOutput(input)
  ...
  self.target = torch.Tensor(self.h*self.w, self.c, self.c):typeAs(input):zero()
  self.G = torch.Tensor(self.target:size()):zero():cuda()
 -- local file  = io.open('log.txt','r')

  for bias = 1, self.h*self.w, self.stride do
    -- For stage one, Just considering stride equals 1.
    -- Maybe Later, we use double for to better stride sampling function.
    -- Using x_stride and y_stride are better ways.
    ...
    self.target[{{bias},{},{}}] = self.gram:forward(mn_patch):clone()
    -- Just for debuging
...
function StyleLoss:updateGradInput(input, gradOutput)
  ...
  local grad_patch = torch.Tensor(self.h*self.w, self.c, self.padding, self.padding):cuda()--(H*W)*C*PAD*PAD
  ...
  --  print('m:'..m..',n:'..n)
    ...
    grad_patch[{{bias},{},{},{}}] = self.gram:backward(mn_patch, dG[bias]):clone() -- C*PAD*PAD
  end
  -- **AVERAGE method**,
